...
def curve_gen(curve_db, curve_type, order_bound, verbose, binary, single_curve):
...
def custom_curves(curve_db, curve_type, order_bound, verbose, binary, single_curve):
    return [c for c in curve_gen(curve_db, curve_type, order_bound, verbose, binary, single_curve)]


# Creates a list of instances of class CustomCurve out of imported database (conditioned by curve_type, see above)
def import_curves(curve_type="sample", order_bound=256, verbose=False, binary = False, single_curve = ""):
    if single_curve != "":
    ...
    curve_list = sorted(custom_curves(curve_db, curve_type, order_bound, verbose, binary, single_curve), key=lambda item: item.order)
    return curve_list
