...
def verify_security(E, embedding_degree_bound=100, verbose=False):
    if verbose:
...
def generate_secg_curves(count, p, seed):
    bits = p.nbits()
    sim_curves = {"name": "secg_sim_" + str(bits), "desc": "simulated curves generated according to the sec standard",
    ...
    with open('secg/parameters_secg.json', 'r') as f:
        params = json.load(f)
