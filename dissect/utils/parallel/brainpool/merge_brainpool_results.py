...
from dissect.definitions import PARALLEL_RESULTS_PATH, BRAINPOOL_PATH
...
BRAINPOOL_RESULTS = os.path.join(PARALLEL_RESULTS_PATH, "brainpool")
VERBOSE = False

# get the names of immediate subdirs, which should be the respective bitsizes
bitsizes = [f.name for f in os.scandir(BRAINPOOL_RESULTS) if f.is_dir()]

for bitsize in bitsizes:
    results_path = os.path.join(BRAINPOOL_RESULTS, bitsize)
    # skip empty dirs
    if len(os.listdir(results_path)) == 0:
        continue

    with open(os.path.join(BRAINPOOL_PATH, "parameters_brainpool.json"), "r") as f:
        params = json.load(f)
        original_seed = params[bitsize][1]
    merged = None

    for root, _, files in os.walk(results_path):
        # iterate through result files, starting with smallest seeds
        for file in sorted(
            files,
            key=lambda x: int((x.split(".")[-2]).split("_")[-1], 16)
        ):
            ...
            with open(fname, "r") as f:
                ...
                if merged is None:
                    ...
                    expected_initial_seed = update_seed(
                        original_seed, merged["seeds_tried"]
                    )
