...
def generate_brainpool_curves(count, p, seed):
    ...
    with open("brainpool/parameters_brainpool.json", "r") as f:
        params = json.load(f)
        original_seed = params[str(bits)]

    field = GF(p)
    seed = update_seed(seed)
    for i in range(1, count + 1):
        a, seed = find_a(field, seed, bits)
        seed = update_seed(seed)
        b, seed = find_b(field, seed, bits)
        if not check_discriminant(a, b, p):
            seed = update_seed(seed)
            continue
        curve = EllipticCurve(field, [a, b])
        ...
        if not security(curve, q):
            ...
            continue
        seed = update_seed(seed)
        k = find_integer(seed, bits, modified=True)
        ...
        sim_curve = {
            ...
            "form": "Weierstrass",
            "params": {"a": {"raw": int_to_hex_string(a)}, "b": {"raw": int_to_hex_string(b)}},
            "generator": {"x": {"raw": int_to_hex_string(x)}, "y": {"raw": int_to_hex_string(y)}},
            "order": q,
            "cofactor": 1,
