...
class CustomCurve:
    ...
    def __init__(self, db_curve):
        ...
        self._ec = None
        self._standard = (
            db_curve.get("standard", False) or "sim" not in db_curve["name"]
        )
    ...
    def embedding_degree(self):
        if self._embedding_degree is None:
            self._embedding_degree = (
                Integers(self._order)(self._q)
    ...
    def frobenius_disc_factorization(self):
        if self._cm_factorization is None:
            ...
            if self._cm_discriminant is not None:
                ...
                self._cm_factorization = (
                    Factorization(self._cm_discriminant) + conductor + conductor
                )
            else:
    ...
    def extended_ec(self, deg):
        ...
        ext_field = GF(
            ext_q,
            name="z",
            modulus=prime_field["z"].irreducible_element(deg * self._field.degree()),
        )
        ...
        new_coeffs = list(
            map(
                lambda x: i(hf(str(x).replace(str(self._field.gen()), "h"))),
                self._ec.a_invariants(),
            )
        )
