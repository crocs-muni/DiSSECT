...
class ZVPFinder:
    ...
    Q = QuotientRing(
        R,
        R.ideal(y1 ** 2 - x1 ** 3 - a * x1 - b, y2 ** 2 - x2 ** 3 - a * x2 - b),
    ...
    def __init__(
        self, formula_file, multiple, verbose=False, one_point_dependent=False
    ):
        ...
        self.zvp_lifted = [
            x.lift()
            for x in self.zvp_set
            if not one_point_dependent
        ...
        self.zvp_substited = [
            x(x2=self.x2_subst, y2=self.y2_subst) for x in self.zvp_sorted
        ]
        ...
        self.zvp_reduced_sorted = sorted(
            self.zvp_reduced_lifted, key=lambda x: len(str(x))
        )
        self.zvp_univariate = self.make_conditions_univariate()
        self.output_polys = [
            x.lift()
            for x in [self.register["X3"], self.register["Y3"], self.register["Z3"]]
        ]
        if verbose:
    ...
    def add_atomic(self, value, zvp_set):
        ...
        if not isinstance(rad, Integer):
            for f, m in rad.factor():
                if (
                    not isinstance(f, Integer)
                    and not isinstance(f, int)
                    and not self.Q(-f) in zvp_set
                ):
    ...
    def print_zvp_conditions(self):
        ...
        print(
            "Thus the ZVP conditions become (after eliminating linear occurences of y1):"
        )
    ...
    def depends_on_one_point(self, p):
        ...
        return (
            self.x1 not in vs
            and self.y1 not in vs
            or self.x2 not in vs
            and self.y2 not in vs
    ...
    def make_conditions_univariate(self):
        ...
        for p in self.zvp_reduced_sorted:
            if self.y1 in p.variables():
                ...
                curve_function = (
                    self.y1 ** 2 - self.x1 ** 3 - self.a * self.x1 - self.b
                ) / 1
    ...
    def find_points(self, a, b, q, verbose=False):
        ...
        if verbose:
            print(
                "\nFinding ZVPs over finite field with",
                q,
                "elements for a =",
                a,
                "and b =",
        ...
        for p in zvp_evaluated_nonconst:
            if p == 0:
                zero_value_points.append(
                    "Any point, since a,b already satisfy a ZVP condition"
                )
            else:
...
def main():
    ...
    formula_file = Path(
        EFD_PATH, "shortw", "projective", "addition", "add-2016-rcb.op3"
    )
