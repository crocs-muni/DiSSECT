...
def i12_curve_function(curve: CustomCurve, unrolled_formula_file):
    ...
    output_polys = [
        ...
        Y3,
        Z3,
    ]  # the other output polynomials such as T3, ZZ3, ZZZ3 are not needed for our purposes
    ...
    if "shortw" in unrolled_formula_file:
        output_polys_converted = [
            pr_clean(pr(poly)(a=shortw_a, b=shortw_b)) for poly in output_polys
        ]
        I = pr_clean.ideal(
            Y1 ** 2 - X1 ** 3 - shortw_a * X1 - shortw_b,
        ...
        output_polys_converted = [
            pr_clean(poly(a=twisted_a, d=twisted_d)) for poly in output_polys
        ]
        I = pr_clean.ideal(
            twisted_a * X1 ** 2 + Y1 ** 2 - (1 + twisted_d * X1 ** 2 * Y1 ** 2),
        ...
        output_polys_converted = [
            pr_clean(poly(c=edwards_c, d=edwards_d)) for poly in output_polys
        ]
        I = pr_clean.ideal(
            X1 ** 2 + Y1 ** 2 - edwards_c * (1 + edwards_d * X1 ** 2 * Y1 ** 2),
