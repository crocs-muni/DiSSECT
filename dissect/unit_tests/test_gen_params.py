...
class TestGenParams(unittest.TestCase):
    ...
    def test_read(self):
        ...
        if "i07" in result:
            self.assertEqual(result["i07"]["params_global"], {}, "Wrong global params for i07, should be " + "{}")
            self.assertEqual(result["i07"]["params_local_names"], [], "Wrong local params for i07, should be " + "[]")
